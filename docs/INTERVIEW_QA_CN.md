...
程序拒绝。项目目前有 67 个 Agent 验收案例，覆盖单轮、多轮、检索和教学行为，
并保留完全离线运行能力。
...
检索不是只靠“看起来相关”验收。22 个 module-scoped case 计算 Hit@3 和 MRR，
当前本地基线为 Hit@3 1.0、MRR 0.9242。
...
67 个 case 是针对本课程的回归测试，只能证明已定义行为没有退化。它不能证明模型
...
6. 打开 `/health` 与 `/api/tools`，说明工程监控和工具参数契约。
