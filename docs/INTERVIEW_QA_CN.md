...
程序拒绝。当前发布门禁保留完全离线运行能力：运行时 `unittest` 317 项，独立浏览器
E2E 11 项，并额外运行课程覆盖、证据充分性、多语言、个性化和 RAG 可信度评测。
...
当前各评测 suite 是针对本课程的回归测试，只能证明已定义行为没有退化。它不能证明模型
...
45. **当前 hard set 99/129 怎么解释？** 是困难可信度诊断，不包装成通用完美检索；独立 holdout 当前为 15/32，两者都与结构化课程覆盖分开报告。
